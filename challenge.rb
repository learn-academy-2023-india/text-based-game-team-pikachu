  ...
  class TextGame
    ...
    def initialize_pokemons
      ...
      def display_pokemon_choices
        ...
        # Display the starting Pokémon
        initialize_pokemons.each_with_index do |(name, pokemon), index|
          puts "#{index + 1}. #{name}"
        end
        # Display the player's captured Pokémon, if any
        unless @player_pokemons.empty?
          @player_pokemons.each_with_index do |pokemon, index|
            puts "#{index + initialize_pokemons.size + 1}. #{pokemon.name}"
          end
        end
        print "Enter the number of your choice (1-#{@player_pokemons.size + initialize_pokemons.size}): "
        choice = gets.chomp.to_i
        if choice.between?(1, initialize_pokemons.size)
          @player_pokemon = initialize_pokemons.values[choice - 1]
        elsif choice.between?(initialize_pokemons.size + 1, initialize_pokemons.size + @player_pokemons.size)
          @player_pokemon = @player_pokemons[choice - initialize_pokemons.size - 1]
        else
